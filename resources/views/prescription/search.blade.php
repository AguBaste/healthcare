    ...
    <x-slot name="header">
        ...
        @if ($prescriptions[0] != null)
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                Recetas de <span
                    class="capitalize">{{ $prescriptions[0]->user->lastname . ' ' . $prescriptions[0]->user->name }}</span>
            </h2>
            <div class="py-12">
                                    ...
                                    <tbody>
                                        @foreach ($prescriptions as $prescription)
                                            <tr>
                                                ...
                                                <td class="capitalize">
                                                    {{ $prescription->user->lastname . ' ' . $prescription->user->name }}
                                                </td>
                                                ...
                                                <td class="p-6"><x-primary-a
                                                        href="{{ route('prescription.edit', $prescription) }}">editar</x-primary-a>
                                                </td>
                                                <td><x-primary-a class="bg-teal-900"
                                                        href="{{ route('prescription.show', $prescription) }}">imprimir</x-primary-a>
                                                </td>
                                                @if ($prescription->created_at != $prescription->updated_at)
                                                    <td class="capitalize">editado
                                                        {{ $prescription->updated_at->diffForHumans() }}</td>
                                                @endif
                                            </tr>
                                        @endforeach
                                    </tbody>
        ...
        @else
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight"> No hay de recetas de este
                paciente</h2>
        @endif
